...
export default function Navbar() {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 w-full md:left-1/2 md:top-6 md:w-max md:-translate-x-1/2 md:rounded-full md:border md:shadow-xl z-50 flex items-center gap-6 md:gap-7 px-5 md:px-6 py-2 transition-all duration-300 ${visible ? "translate-y-0" : "-translate-y-full md:-translate-y-[calc(100%+1.5rem)]"} ${menuAbierto ? "bg-black" : ultimoScroll > 80 ? "bg-black/40 backdrop-blur-md" : "bg-transparent"} md:bg-transparent! md:backdrop-blur-lg md:backdrop-saturate-150 md:border-white/8`}
      >
        <Link
          to="/"
          onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
          className="border-none bg-transparent p-0 cursor-pointer relative z-50 shrink-0"
        >
        ...
        <ul className="hidden md:flex gap-8 list-none whitespace-nowrap">
          <li>
            <NavLink
              to="/horarios"
              className={({ isActive }) =>
                `text-base no-underline transition-colors duration-200 ${isActive ? "text-brand-gold" : "text-white hover:text-brand-gold"}`
              }
            >
              Horarios
            </NavLink>
        ...
        <button
          ...
          className={`md:hidden ml-auto relative w-10 h-10 z-50 flex items-center justify-center transition-all duration-300 
                    ${
                      menuAbierto
        ...
        >
          ...
          <span
            className={`absolute h-0.5 w-6 bg-current transition-all duration-300 ease-in-out origin-center
                    ${menuAbierto ? "opacity-0" : "top-1/2 -translate-y-1/2"}`}
          />
